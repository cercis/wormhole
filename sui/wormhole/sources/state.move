module wormhole::state {
    ...
    public entry fun set_governance_action_consumed(has: vector<u8>){
    }
    ...
    public(friend) fun update_guardian_set_index(new_index: U32) {
    ...
    public(friend) fun expire_guardian_set(index: U32) {
        //let state = borrow_global_mut<WormholeState>(@wormhole);
    ...
    public(friend) fun store_guardian_set(set: GuardianSet) {
        //let state = borrow_global_mut<WormholeState>(@wormhole);
        //let index: u64 = u32::to_u64(structs::get_guardian_set_index(&set));
        //table::add(&mut state.guardian_sets, index, set);
    }

    public fun guardian_set_is_active(guardian_set: &GuardianSet): bool {
